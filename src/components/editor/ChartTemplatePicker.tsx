...
export default function ChartTemplatePicker({
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, ChartTemplate[]>()
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-medium text-gray-700">Escolha um template de gráfico</p>
      </div>
      ...
      >
        ...
        <div>
          <p className="font-medium text-gray-900 text-sm">Gráfico Vazio</p>
          <p className="text-xs text-gray-500 mt-0.5">Criar gráfico em branco sem template</p>
        </div>
      </button>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        {tpl.chartType === 'bar' ? 'Barras' : 'Linhas'}
                        {' · '}
                        {tpl.categories.length} {tpl.categories.length === 1 ? 'categoria' : 'categorias'}
                        {tpl.referenceRegions.length > 0 && ' · com regiões de referência'}
                      </p>
                    </div>
                  </button>
                ))}
